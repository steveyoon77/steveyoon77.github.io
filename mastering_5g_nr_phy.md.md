...
### 2️⃣ 다중경로 전파 (Multipath Propagation)
...
### 3️⃣ OFDM이 Multipath 문제를 해결하는 방법
...
### 3️⃣ Physical Resource Block (PRB)
...
### 🧩 **2️⃣ SSB vs SS Burst vs SS Burst Set**
...
### 🧱 **6️⃣ SS Burst의 프레임 내 배치 (Time–Frequency Structure)**
...
#### 2️⃣ 시퀀스 초기화 식 (3GPP TS 38.211 §7.4.1.4)
$$
...
### 🧠 **4️⃣ PDCCH 구조 (Time–Frequency Plane)**
...
### 📊 **8️⃣ PDCCH 관련 파라미터 (3GPP 38.211/38.213)**
...
### 📘 **9️⃣ 예시 (Downlink Scheduling Flow)**

1️⃣ gNB가 UE에 PDSCH 자원을 할당해야 함  
2️⃣ DCI(포맷 1_0) 생성  
3️⃣ DCI를 CCE에 매핑하여 PDCCH로 전송  
4️⃣ UE가 Search Space에서 해당 PDCCH를 탐색 및 디코딩  
5️⃣ DCI 정보로 PDSCH 위치·MCS·HARQ 설정  
6️⃣ PDSCH에서 데이터 수신
...
### 🔐 **7️⃣ RNTI (Radio Network Temporary Identifier)**
...
### 🧠 **5️⃣ CORESET의 주요 파라미터 (3GPP 38.211 §7.3.2.2)**
...
### 🧩 **1️⃣0️⃣ DCI 수신 절차에서 CORESET의 역할**

1️⃣ UE는 RRC 메시지로 CORESET 구성을 수신  
2️⃣ Search Space 구성을 통해 탐색 영역 설정  
3️⃣ CORESET 내에서 PDCCH 탐색 수행  
4️⃣ CRC 마스크로 자신의 RNTI 확인  
5️⃣ 해당 DCI 해석 → 자원 할당 수행
...
### 🧭 **5️⃣ 실제 예시 (3GPP NR 표준에서 자주 쓰는 CORESET 크기)**
...
### 🧩 **9️⃣ Search Space와 CORESET 관계 정리**
...
### ⚙️ **3️⃣ Blind Decoding의 과정 요약**

1️⃣ UE는 **Search Space** 설정에 따라 후보 위치(=CCE 집합)들을 알고 있음.  
2️⃣ 각 후보에 대해, 가능한 **Aggregation Level** (1, 2, 4, 8, 16)을 적용.  
3️⃣ 각각의 후보 CCE 묶음에 대해 **DCI 디코딩 시도** 수행.  
4️⃣ 디코딩된 DCI의 **CRC를 RNTI로 마스크 해제**.  
5️⃣ CRC 결과가 자신의 RNTI와 일치하면 → 성공!  
6️⃣ 일치하지 않으면 → 다음 후보로 이동.
...
#### 2️⃣ 코드 블록 분할 (Code Block Segmentation)
...
### 🧮 **4️⃣ TBS Calculation Step-by-Step (3GPP TS 38.214 §5.1.3.2)**
...
### 📊 **7️⃣ TBS Table 개요 (3GPP 38.214, Table 5.1.3.2-1)**
...
### 📊 **6️⃣ Precoding Matrix Index (PMI)**
...
1️⃣ UE가 PDSCH 수신  
2️⃣ CRC → Pass → ACK, Fail → NACK  
3️⃣ UCI = 1 bit로 생성  
4️⃣ gNB가 미리 구성한 **PUCCH Format 0 Resource**에 맵핑  
5️⃣ BPSK 변조 → DMRS 포함 → 송신  
6️⃣ gNB는 해당 PUCCH 자원에서 ACK/NACK 수신 및 복조
...
### ⚙️ **CSI-RS 전송 유형 (3GPP 38.211 §7.4.1.5)**
...
1️⃣ **gNB → UE**: CSI-RS 전송  
2️⃣ **UE**: CSI-RS 수신 후 채널 추정  
3️⃣ **UE → gNB**: 피드백 보고 (PUSCH 또는 PUCCH)
...
1️⃣ **gNB → UE**: RRC(Reconfiguration)로 CSI-RS & CSI-IM 자원 설정  
2️⃣ **UE**:

- CSI-RS 구간에서 신호 세기 (P_S) 측정
- CSI-IM 구간에서 간섭 (P_I) 측정  
    3️⃣ **UE**: SINR = ( P_S / P_I ) 계산  
    4️⃣ **UE → gNB**: CQI, PMI, RI 보고 (PUCCH/PUSCH 경로)
...
1️⃣ UE는 CSI-RS와 CSI-IM으로부터 채널 품질(SINR)을 계산:  
$$
\text{SINR} = \frac{P_{\text{CSI-RS}}}{P_{\text{CSI-IM}}}  
$$

2️⃣ UE는 3GPP 표준의 **CQI → MCS 매핑 테이블**을 사용해 적절한 CQI 값을 선택.
...
1️⃣ UE는 gNB가 송신한 CSI-RS를 수신하고, 채널 응답 행렬 $\mathbf{H}$ 추정:  
$$
\mathbf{H} =  
\begin{bmatrix}  
...
2️⃣ 표준화된 **Codebook**(3GPP 정의)을 이용해 후보 precoding 행렬 $\mathbf{W}_i$ 들 중에서  
다음 목표를 만족하는 행렬을 선택:  
$$
\mathbf{W}_{opt} = \arg\max_{i} | \mathbf{H} \mathbf{W}_i |^2  
$$

3️⃣ 그 행렬의 인덱스 = **PMI (Precoding Matrix Indicator)**  
...
1️⃣ UE는 CSI-RS를 통해 채널 행렬 ( H ) 추정  
2️⃣ SVD(특이값 분해) 수행:  
$$  
H = U \Sigma V^H  
$$  
3️⃣ 행렬 $\Sigma$ 의 대각 원소(특이값) 중 유효한 개수(count)가 rank입니다.
$$
RI = \text{rank}(H)  
$$
4️⃣ UE는 rank에 해당하는 layer 수(1~8)를 보고합니다.
